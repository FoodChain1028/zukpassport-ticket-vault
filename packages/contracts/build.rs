...
#[cfg(all(
    feature = "build",
    not(any(feature = "ticket-app", feature = "contract2"))
))]
fn main() {
    compile_error!("When the 'build' feature is enabled, at least one of the following features must also be enabled: all, ticket-app, contract2.");
}

#[cfg(all(
    not(clippy),
    feature = "build",
    any(feature = "ticket-app", feature = "contract2",)
))]
fn main() {
    trait CodegenConsts {
