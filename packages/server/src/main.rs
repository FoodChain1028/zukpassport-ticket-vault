...
pub struct Args {
    ...
    pub ticket_app_cn: String,

    #[arg(long, default_value = "hyllar")]
    pub hyllar_cn: String,

    #[arg(long, default_value = "hydentity")]
    pub hydentity_cn: String,
...
async fn main() -> Result<()> {
    ...
    let contracts = vec![init::ContractInit {
        name: args.ticket_app_cn.clone().into(),
        program_id: ticket_app::client::tx_executor_handler::metadata::PROGRAM_ID,
        initial_state: TicketApp::default().commit(),
    }];
    ...
    let app_ctx = Arc::new(AppModuleCtx {
        common: ctx.clone(),
        node_client,
        ticket_app_cn: args.ticket_app_cn.clone().into(),
        hyllar_cn: args.hyllar_cn.clone().into(),
        hydentity_cn: args.hydentity_cn.clone().into(),
    });
    ...
    handler
        .build_module::<ContractStateIndexer<TicketApp>>(ContractStateIndexerCtx {
            contract_name: args.ticket_app_cn.into(),
            common: ctx.clone(),
        })
        .await?;

    handler
        .build_module::<ContractStateIndexer<hyle_hyllar::Hyllar>>(ContractStateIndexerCtx {
            contract_name: args.hyllar_cn.into(),
            common: ctx.clone(),
        })
        .await?;

    handler
        .build_module::<ContractStateIndexer<hyle_hydentity::Hydentity>>(ContractStateIndexerCtx {
            contract_name: args.hydentity_cn.into(),
            common: ctx.clone(),
        })
        .await?;
